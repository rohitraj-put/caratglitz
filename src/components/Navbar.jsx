...
function Navbar({ send, wishCount }) {
  ...
  const handleInputChange = (e) => {
    ...
    if (value) {
      ...
      const uniqueCategories = Array.from(
        new Set(filteredSuggestions.map((item) => item.category))
      );
  ...
  const countryFlags = {
    India:
      "https://static.vecteezy.com/system/resources/previews/016/328/568/original/india-flat-rounded-flag-with-transparent-background-free-png.png",
    Dubai:
      "https://cdn2.iconfinder.com/data/icons/world-flags-1-1/100/uae-512.png",
    "New York":
      "https://www.pngall.com/wp-content/uploads/12/USA-Flag-PNG-File.png",
    London:
      "https://www.pngall.com/wp-content/uploads/12/USA-Flag-PNG-File.png",
    Harare:
      "https://static.vecteezy.com/system/resources/previews/015/272/144/non_2x/zimbabwe-3d-rounded-flag-with-transparent-background-free-png.png",
    Italy:
      "https://static.vecteezy.com/system/resources/previews/011/571/348/non_2x/circle-flag-of-italy-free-png.png",
  };
    ...
    <>
      <header className="flex sticky top-0 z-20 bg-white border-b py-2 sm:px-8 px-2 font-sans min-h-80px tracking-wide">
        <div className="flex flex-wrap items-center lg:gap-y-2 gap-4 w-full">
          ...
          <div
            className={`lg:ml-10 ${isMenuOpen ? "block" : "hidden"} lg:block`}
          >
            <ul className="lg:flex lg:gap-x-3 max-lg:space-y-3 max-lg:fixed max-lg:bg-white max-lg:w-1/2 max-lg:min-w-300px max-lg:top-0 max-lg:left-0 max-lg:p-6 max-lg:h-full max-lg:shadow-md max-lg:overflow-auto z-50">
              <li className="mb-6 hidden max-lg:block">
                <div className="flex justify-between">
                  ...
                  <button
                    ...
                    id="toggleClose"
                    className="lg:hidden z-100 rounded-full"
                  >
              ...
              {NavData.map((item, index) => (
                <li
                  key={index}
                  className="max-lg:border-b max-lg:py-3 px-3  lg:hidden"
                >
                  <Link
                    to={`${item.link}`}
                    className="hover:text-rose-400 text-15px block font-semibold"
                  >
                    {item.label}
                  </Link>
                </li>
          ...
          <div className="flex gap-x-6 gap-y-4 ml-auto">
            <div className="flex items-center space-x-4">
              ...
              <div className="relative">
                <img
                  className="w-10 h-10 rounded-full object-cover cursor-pointer z-50"
                  src={
                    data?.avatar
                      ? data.avatar
                  ...
                  <ul className="absolute right-0 mt-2 w-48 overflow-hidden bg-white border rounded-lg shadow-lg">
                    <li className="px-4 py-2  text-rose-400 bg-gray-100">
                      <p className=" capitalize font-bold">
                        Welcome, {data?.username ? data.username : "Guest"}
                      </p>
                    </li>
      ...
      <div className="fixed z-10 w-14 h-14 rounded-full bottom-16 right-4 bg-rose-400 p-2.5 hover:bg-rose-500">
        <a
          href="https://web.whatsapp.com/send?l=en&phone=+91 8506922777&text=I want to order a website"
          target="_blank"
